<#
	...
	Version:	Release 1.01 - Published: 04 Sep 2021
...
{
	$request = $param[0]
	$basedir = "/home/$env:USER/msupdate-dl/"
	...
	if ($request -eq "-version") {
		...
		Write-Host "Release 1.00 - 04 Sep 2021" -ForegroundColor Magenta
	...
	if ($details -eq $true) {
		...
		wget https://www.catalog.update.microsoft.com/Search.aspx?q=$request --user-agent="Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.1; WOW64; SLCC2; .NET CLR 2.0.50727; .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; .NET4.0C; InfoPath.2)" --output-document $infofile
	...
	foreach ($kbGUID in $kbGUIDs) {
		...
		if ($details -eq $true) {
			...
			do {
				...
				# call the HTTP retriever. wget used in this project
				wget https://www.catalog.update.microsoft.com/ScopedViewInline.aspx?updateid=$kbGUID --user-agent="Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 6.1; WOW64; SLCC2; .NET CLR 2.0.50727; .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; .NET4.0C; InfoPath.2)" --output-document $infofile
		...
		ForEach-Object { 
			foreach ($line in $_.matches.value) {
				...
				if (($condition1 -eq $true) -and ($condition2 -eq $true) -and ($condition3 -eq $true)) {
					...
					# call the HTTP retriever. wget used in this project
					wget $line --no-clobber --directory-prefix=$outdir
				}
			}
		}
	}
